...
function initAnchors() {
	new SmoothScroll({
		anchorLinks: 'a.link-down[href^="#"]:not([href="#"]), a.contact-links-box[href^="#"]:not([href="#"])',
		wheelBehavior: 'none',
...
function initSlickCarousel() {
	jQuery('.main-slider').slick({
		...
		prevArrow: '<button type="button" class="slick-prev"><i class="ti-angle-left"></></button>',
		nextArrow: '<button type="button" class="slick-next"><i class="ti-angle-right"></></button>',
		adaptiveHeight: true,
		autoplay: false
	});
	...
	ResponsiveHelper.addRange({
		'720..': {
			...
			off: function() {
				jQuery('.desktop-slider').each(function() {
...
function initVimeoBackgroundVideo() {
	// detect device type
	var isTouchDevice = /Windows Phone/.test(navigator.userAgent) || ('ontouchstart' in window) || window.DocumentTouch && document instanceof DocumentTouch;

	if (isTouchDevice) return;

	jQuery('.bg-video-vimeo:has([data-url])').each(function() {
		var holder = jQuery(this);
		var vimeoPlaceholder = holder.find('[data-url]');
		var url = vimeoPlaceholder.attr('data-url');

		vimeoPlaceholder.attr('data-vimeo-url', url);

		var player = new Vimeo.Player(vimeoPlaceholder[0]);

		player.ready().then(function() {
			var iframeObj = holder.find('iframe');

			holder.addClass('player-ready');

			iframeObj.data('player', player);


			if (holder.hasClass('video-parallax')) {
				...
				VideoStretcher.add({
					container: vimeoPlaceholder,
					video: holder.find('iframe')
				});
			}
	...
	function onPlayerReady() {
		jQuery('.js-vimeo-slider').each(function() {
			var slider = jQuery(this);

			slider.on('beforeChange', function(e, slick, currIndex) {
				var iframeObj = slick.$slides.eq(currIndex).find('[src*="https://player.vimeo.com"]');
				var player = iframeObj.data('player');

				player && player.pause();
			}).on('afterChange', function(e, slick, currIndex) {
				var iframeObj = slick.$slides.eq(currIndex).find('[src*="https://player.vimeo.com"]');
				var player = iframeObj.data('player');
	...
	var ParallaxController = (function() {
		...
		return {
			...
			calculateSize: function(i) {
				...
				if (item.$video) {
					item.$video.css({
						width: Math.round(styles.width) + 'px ',
						height: Math.round(styles.height) + 'px'
					});
				} else {
					item.$el.css({
			...
			calculateScroll: function(i) {
				...
				if (winProps.scrollTop + winProps.height > item.topOffset && winProps.scrollTop < item.topOffset + item.height) {
					...
					if (item.$video) {
						item.$video.css({
							top: -1 * (winProps.height * (item.options.parallaxOffset / 100) - (winProps.height + item.height) * ratio * (item.options.parallaxOffset / 100)) + 'px'
						});
					} else {
						item.$el.css({
			...
			videoLoaded: function(iframe, callback) {
				var self = this;
				var loadHandler = function() {
					callback.call(self);
				};
			...
			add: function(el, options) {
				...
				var $video = $el.find(options.video);

				if ($video.length) {
					this.videoLoaded($video, function() {
						var videoRatio = this.getVideoRatio($video);
						...
						options.parallaxOffset = 100 - Math.abs(options.parallaxOffset);

						var newIndex = items.push({
							$el: $el,
							$video: $video,
							options: options,
							imageRatio: videoRatio
						});
